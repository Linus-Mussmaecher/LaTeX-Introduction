...
Hier zu existiert beispielsweise die Package \latexpackage{fixme}.
Als grundlegendstes Command stellt diese das Command \latexcommand{fxnote} bereit:
\begin{latexlisting}
	...
	\fxnote{Wobei ich mir denk sie hätte sich besser mal die Frage gestellt: Wie weit kommt man denn mit 14, und ohne Geld?}
\end{latexlisting}
Diese Anmerkung taucht dabei im PDF nur auf, wenn die Kompilieurng im \latexargument{draft}-Modus stattfindet.
Zusätzlich zu \latexcommand{fxnote} existieren verschiedene Levels an Anmerkungen, die durch die Commands \latexcommand{fxwarning}, \latexcommand{fxerror} und \latexcommand{fxfatal} ausgedrückt werden.
Insbesondere erlaubt \latexcommand{fxfatal} es nicht, das Dokument zu kompilieren, bevor der Fehler gefixt wurde.
...
\begin{latexlisting}
	...
	Sie verfehlt knapp ihr Herz aber \fxnote*{Sehr schmerzhaft! Nicht nachmachen!}{trifft immerhin ihren Bauch}.\\
...
\end{latexlisting}
anzuzeigen.
Die Package \latexpackage{fixme} besitzt noch einige andere nützliche Features, unter anderem das hinzufügen mehrerer Autoren oder Korrekturleser, denen Anmerkungen zugeordnet werden können, um kollaboratives Arbeiten zu ermöglichen.
